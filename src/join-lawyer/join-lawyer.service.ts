...
export class JoinLawyerService {
  ...
  private get uploadRoot() {
    return process.env.UPLOAD_ROOT || '/data/uploads';
  }
  ...
  private getPublicBarCouncilIdUrl(appId: number) {
    return `/api/join-lawyer/applications/${appId}/bar-council-id`;
  }

  private getPlanMeta(selectedPlan?: string | null) {
    const normalized = (selectedPlan || '').trim().toLowerCase();

    const planMap: Record<string, { planYears: number | null; amountInr: number | null }> = {
      starter: { planYears: 1, amountInr: 1999 },
      growth: { planYears: 2, amountInr: 3499 },
      pro: { planYears: 3, amountInr: 4999 },
    };

    return planMap[normalized] || { planYears: null, amountInr: null };
  }

  private mapApplication(app: JoinLawyerEntity) {
    const { planYears, amountInr } = this.getPlanMeta(app.selectedPlan);

    return {
      ...app,
      name: `${app.firstName || ''} ${app.lastName || ''}`.trim() || null,
      mobile: app.phone || null,
      phoneNumber: app.phone || null,
      phoneCountryCode: app.code || null,
      fullPhone: `${app.code || ''}${app.phone || ''}` || null,
      planYears,
      amountInr,
      photoUrl: app.photoPath ? this.getPublicPhotoUrl(app.id) : null,
      barCouncilIdUrl: app.barCouncilIdPath
        ? this.getPublicBarCouncilIdUrl(app.id)
        : null,
    };
  }
  ...
  async setBarCouncilId(
    ...
    return {
      success: true,
      url: this.getPublicBarCouncilIdUrl(id),
    };
  }
